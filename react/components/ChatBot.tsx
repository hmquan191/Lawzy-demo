...
const ChatBot = () => {
  ...
  const handleSend = async () => {
    ...
    const userMessage: Message = { from: "user", text: input, timestamp: new Date() };
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-72' : 'w-0'} transition-all duration-300 overflow-hidden flex flex-col border-r border-gray-700 bg-gray-800`}>
        ...
        <div className="p-2 border-b border-gray-700">
          ...
          <div className="space-y-1">
            {chatHistories.map(chat => (
              <div 
                key={chat.id} 
                className={`p-2 rounded cursor-pointer hover:bg-gray-700 ${activeChatId === chat.id ? 'bg-gray-700' : ''}`}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-gradient-to-b from-gray-900 to-gray-800">
          {messages.map((msg, idx) => (
            <div
              key={idx}
              className={`my-4 flex ${msg.from === "user" ? "justify-end" : "justify-start"}`}
            >
              {msg.from === "bot" && (
              ...
              >
                {msg.from === "user" ? (
                  ...
                  <div>
                    <div 
                      className="prose prose-invert max-w-none prose-headings:my-2 prose-p:my-2 prose-ul:my-2 prose-li:my-1"
                      dangerouslySetInnerHTML={{ __html: markdownToHTML(msg.text) }}
                    />
                    <div className="text-xs text-gray-400 text-right mt-1">{formatTime(msg.timestamp)}</div>
                  </div>
                )}
              </div>
              {msg.from === "user" && (
      ...
      <div className={`w-80 border-l border-gray-700 bg-gray-800 transition-all duration-300 overflow-hidden ${showLawyers ? 'translate-x-0' : 'translate-x-full'} absolute right-0 top-0 bottom-0`}>
        <div className="p-4 border-b border-gray-700 flex items-center justify-between">
            ...
            <div key={lawyer.id} className="bg-gray-700 rounded-lg p-3">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  />
                  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full ${lawyer.online ? 'bg-green-500' : 'bg-gray-500'}`}></div>
                </div>
              ...
              <button 
                ...
                className={`mt-3 w-full py-1.5 rounded text-sm ${lawyer.online ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-600 cursor-not-allowed'}`}
